...
import {
  ...
  Send, CheckCircle, Loader2, Database, Code2, Clock, ArrowRight, Newspaper
} from "lucide-react"
...
function LatestNewsSection({ articles }: { articles: any[] }) {
    ...
    <section className="max-w-6xl mx-auto px-6 mt-12 mb-12">
      <div className="flex items-center justify-between mb-8">
        <h2 className="text-3xl font-black text-white tracking-tight">
          LATEST <span className="text-transparent bg-clip-text bg-gradient-to-r from-[#ff007f] to-[#00ffff]">NEWS</span>
        </h2>
        <Link href="/news" className="text-white/50 hover:text-white transition-colors text-sm font-bold flex items-center gap-2 uppercase tracking-wider">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        {latestArticles.map((article: any) => (
          <Link
            ...
            className="group glass rounded-3xl overflow-hidden hover:border-[#00ffff]/50 transition-all duration-300 flex flex-col h-full bg-[#121216]/60 backdrop-blur-md border border-white/5"
          >
            <div className="aspect-video w-full relative overflow-hidden bg-black/50">
              {article.image_url ? (
                ...
                <div className="w-full h-full bg-gradient-to-br from-[#ff007f]/20 to-[#00ffff]/20 flex items-center justify-center">
                  <span className="text-white/20 font-black text-xl tracking-widest uppercase">LEONIDA</span>
                </div>
              )}
              <div className="absolute top-4 left-4">
                <span className="px-3 py-1 rounded-full bg-black/80 backdrop-blur-md text-white text-[10px] font-bold uppercase tracking-widest border border-white/10">
                  News
                </span>
              </div>
            </div>
            ...
            <div className="p-5 flex flex-col flex-1">
              ...
              <h3 className="text-lg font-bold text-white leading-tight mb-4 group-hover:text-[#00ffff] transition-colors line-clamp-2">
                {article.title}
              </h3>

              <div className="mt-auto flex items-center gap-2 text-[#ff007f] text-xs font-bold uppercase tracking-wider group-hover:gap-3 transition-all">
